...
@onready
var fish_scene = get_node("res://Creatures/FlyingTriangle.tscn")
...
@onready var center_path = get_node("../Center")
...
func draw_gizmos():
	...
	var new_boundary = Boundary.instantiate()
	new_boundary.global_position = Vector3.ZERO
	get_tree().root.add_child(new_boundary)
